# -*- coding: utf-8 -*-
"""
Created on Tue Dec  7 17:36:57 2021
...
num_iter = 60000#100000 #max DNN iteations 
...
for Re_tau in Re_tau1:
    ...
    tau = nu_non_fic[1:]*U_x_train
    
    
    
    U_loss = tf.square(U_train[0] - U_plus[0])  + tf.square(U_train[-1] - spl_U(yp_train[-1]))
    #uv_loss = tf.square(U_x_train[0] - tau[0] - uv_plus[0])  + tf.square(U_x_train[-1] - tau[-1] - uv_plus[-1])
    #uv_Nf_loss = tf.abs(uv_Nf[0])
    tau_loss = tf.square(tau[0] - rhs[0])  + tf.square(tau[-1] - rhs[-1])
    grad_U_loss = tf.square(U_x_train[0] - 1)  + tf.square(U_x_train[-1])
    nu_non_fic_loss =  tf.square(nu_non_fic[1] -1) + tf.square(nu_non_fic[-1])
    
    loss = 100*tf.reduce_mean(tf.square(tau - rhs[1:])) + U_loss + tau_loss + grad_U_loss + nu_non_fic_loss
