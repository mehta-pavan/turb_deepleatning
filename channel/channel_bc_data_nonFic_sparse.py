...
num_iter = 50000#100000 #max DNN iteations 
...
for sprse in sparese:
    ...
    for Re_tau in Re_tau1:
        ...
        y_spasse_data = np.linspace(0, new_Re_tau, num=int(new_Re_tau*3*sprse), endpoint = True)
        yp_train = np.linspace(0.01, new_Re_tau-0.01, num=int(new_Re_tau*3), endpoint = True)
        ...
        dnn_out1 = DNN((y_spasse_data.reshape(-1,1)), layers, weights, biases) #fractional order - aplha
        ...
        uv_Nf = -tf.abs(uv_nf_strain ) #+ nu_non_fic[1:])
        eq1 = U_x_train - uv_Nf
